...
AngularJS's `ngRoute` has one serious limitation in that you can't have nesteds (`ng-view`) in your app. --> **Angular UI Router** is a 3<sup>rd</sup>-party routing module that has many awesome features, including nested view support.

## Getting Started With UI Router

Instead of working with <strike>**URLs**</strike>, Angular UI Router takes a **state-based** approach.

> A state describes how your UI looks at a particular time. To put it simply:
> > A State = a URL + a Controller + a Template

[Installation & Documentation here](http://angular-ui.github.io/ui-router/ "here").
...
angular.module('myApp').config(function ($stateProvider, $urlRouterProvide) {
